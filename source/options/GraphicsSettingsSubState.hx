...
<<<<<<< HEAD
		...
		if (ClientPrefs.screenRes == "FULLSCREEN")
		{
			var option:Option = new Option('Scale Mode',
				...
				'screenScaleModeTemp', 'string', 'LETTERBOX', ['LETTERBOX', 'PAN', 'STRETCH']);
			addOption(option);
		}
		else
		{
			var option:Option = new Option('Scale Mode',
				...
				'screenScaleModeTemp', 'string', 'LETTERBOX', ['LETTERBOX', 'PAN', 'STRETCH', 'ADAPTIVE']);
			addOption(option);
		}
	...
	override function update(elapsed:Float)
	{
		if (controls.ACCEPT)
		{
			if (curOption.name == "Screen Resolution")
			{
				...
				if (ClientPrefs.screenRes == "FULLSCREEN" && ClientPrefs.screenScaleMode == "ADAPTIVE")
					ClientPrefs.screenScaleMode = "LETTERBOX";
				onChangeRes();
			...
			else if (curOption.name == "Scale Mode")
			{
				...
				if (ClientPrefs.screenScaleMode == "ADAPTIVE")
					onChangeRes();
				if (shouldReset)
					MusicBeatState.switchState(new options.OptionsState());
				else
