...
class MovieClip extends Sprite #if (openfl_dynamic && haxe_ver < "4.0.0") implements Dynamic<DisplayObject> #end
{
	...
	**/
	public var framesLoaded(get, never):Float;
	...
	**/
	...
	@:noCompletion private override function __enterFrame(deltaTime:Float):Void
	{
		if (__timeline != null)
		{
			__timeline.__enterFrame(Std.int(deltaTime));
		}

		for (child in __children)
		{
			child.__enterFrame(Std.int(deltaTime));
		}
	}
	...
	@:noCompletion private function get_framesLoaded():Float
	{
		if (__timeline != null)
		{
	...
	@:noCompletion private function get_totalFrames():Float
	{
		if (__timeline != null)
		{
