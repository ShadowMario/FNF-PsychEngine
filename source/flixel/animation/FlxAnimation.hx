...
class FlxAnimation extends FlxBaseAnimation
{
	 ...
	 */
	public var delay(get, default):Float = 0;

	function get_delay()
	{
		/*if (frameDelay[curFrame] != null)
			return frameDelay[curFrame]; */
		@:privateAccess
		if (parent._sprite.frames.frames[curIndex] != null)
		{
			if (parent._sprite.frames.frames[curIndex].delay!=null)
				return parent._sprite.frames.frames[curIndex].delay;
		}
	...
	/**
	 * Sets the per-frame delays, if applicable
	 * Mainly used for Aseprite stuff
	 */
	public var frameDelay:Array<Null<Float>> = [];
