...
class Conductor
{
	...
	public static function judgeNote(note:Note, diff:Float=0) //STOLEN FROM KADE ENGINE (bbpanzu) - I had to rewrite it later anyway after i added the custom hit windows lmao (Shadow Mario)
	{
		...
		var timingWindows:Array<Int> = [ClientPrefs.sickWindow, ClientPrefs.greatWindow, ClientPrefs.goodWindow, ClientPrefs.badWindow];
		var windowNames:Array<String> = ['sick', 'great', 'good', 'bad'];
