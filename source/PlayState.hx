...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (!ClientPrefs.maxOptimization) { // l1ttleO: indenting this will take too long
		switch (curStage)
		{
			case 'stage': //Week 1
		...
		if (curStage == 'limo' && !ClientPrefs.maxOptimization)
			add(limo);

		add(dadGroup);
		add(boyfriendGroup);

		if(curStage == 'spooky' && !ClientPrefs.maxOptimization) {
			add(halloweenWhite);
		}
		...
		if (isStoryMode && !seenCutscene && !ClientPrefs.maxOptimization)
		{
			switch (daSong)
			{
				case "monster":
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if(!practiceMode && !cpuControlled) {
			...
			scoreTxt.scale.x = 1.0;
			scoreTxt.scale.y = 1.0;
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName)) {
				...
				switch(achievementName)
				{
					...
					case 'toastie':
						if((/*ClientPrefs.framerate <= 60 &&*/ ClientPrefs.lowQuality && !ClientPrefs.globalAntialiasing && !ClientPrefs.imagesPersist) || ClientPrefs.maxOptimization) {
							unlock = true;
						}
					case 'debugger':
