...
class PlayState extends MusicBeatState
{
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		...
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				// MusicBeatState.switchState(new GameOverState(char.getScreenPosition().x, char.getScreenPosition().y));
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && startedCountdown && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			...
			if(!char.stunned && generatedMusic && !endingSong)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					if (daNote.canBeHit && daNote.mustPress == !opponentPlay && !daNote.tooLate && daNote.wasGoodHit == opponentPlay && !daNote.isSustainNote && !daNote.blockHit)
					{
	...
	private function keyShit():Void
	{
		...
		if (startedCountdown && !char.stunned && generatedMusic)
		{
			...
			else if (char.holdTimer > Conductor.stepCrochet * (0.0011 / FlxG.sound.music.pitch) * char.singDuration && char.animation.curAnim.name.startsWith('sing') && !char.animation.curAnim.name.endsWith('miss'))
			{
				char.dance();
				//char.animation.curAnim.finish();
			}
