...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Nedovoljno', 0.5], //From 40% to 49%
		['Dovoljno', 0.6], //From 50% to 59%
		['Dobro', 0.69], //From 60% to 68%
		['Vrlo Dobro', 0.7], //69%
	...
	{
		if(ratingName == '?') {
			scoreTxt.text = 'BS: ' + nps + ' | Rezultat: ' + songScore + ' | Greske: ' + songMisses + ' | Koncentricnost: 0% | N/A';
		} else {
			scoreTxt.text = 'BS: ' + nps + ' | Rezultat: ' + songScore + ' | Greske: ' + songMisses + ' | Koncentricnost: ' + Math.floor(ratingPercent * 100) + '% | ' + ratingFC;
		}
		if(cpuControlled) {
			scoreTxt.text = 'Pokazivanje | Robotska Mehanika ';
		}
		if(practiceMode) {
			scoreTxt.text = 'BS: ' + nps + ' | Greske: ' + songMisses + ' | Praktikol Mod ';
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			// Rating FC
			ratingFC = "";
			if (sicks > 0) ratingFC = "(Bolesni Celi Kombo) Bolesno!";
			if (goods > 0) ratingFC = "(Odlican Celi Kombo) Good!";
			if (bads > 0 || shits > 0) ratingFC = "(Celi Kombo) Odlicno";
			if (songMisses > 0 && songMisses < 10) ratingFC = "(Nemate Nijedan Kombo) Dobro";
			if (songMisses >= 10) ratingFC = "(Cisto) Okej";
			if (songMisses >= 30) ratingFC = "(Cisto) Dovoljan";
			if (songMisses >= 65) ratingFC = "(Cisto - Isus Skila) Isus Skila";
			if (songMisses >= 500) ratingFC = "(What The Freak) WTFreak";
			else if (songMisses >= 1000) ratingFC = "Damn";
