...
class RegisterState extends MusicBeatState
{
    var usernameBox:FlxInputText;
    var passwordBox:FlxInputText;
    var registerButton:FlxButton;
    ...
    private function submitCredentials(username:String, password:String)
        {
            /*var resData = ServerConnectionsManager.register(username, password);*/
            var resData = null;
                if(resData != null)
                {
                    if(resData.success)
                    {
                        ...
                        errorTimer.cancel();
                        errorText.text = 'Thanks you for creating an account! Your username is ' + username;
                        errorText.alpha = 1;
    ...
    override function create()
    {
        ...
        registerButton = new FlxButton(FlxG.width / 2 - 200, (passwordBox.y + 150) * 3, "Register", () -> submitCredentials(usernameBox.text, passwordBox.text));
        registerButton.setGraphicSize(70);
        registerButton.updateHitbox();
        //registerButton.loadGraphic(Paths.image("buttons/loginButton", "shared"));
        registerButton.label.setFormat("VCR OSD Mono", 70, FlxColor.BLACK);
        //registerButton.label.y += 100;
        add(registerButton);

        var loginButton:FlxButton = new FlxButton(FlxG.width / 2 + 300, (passwordBox.y + 150) * 3, "Login", () -> LoadingState.loadAndSwitchState(new LoginState()));
        loginButton.setGraphicSize(30);
        loginButton.updateHitbox();
        //loginButton.loadGraphic(Paths.image("buttons/loginButton", "shared"));
        loginButton.label.setFormat("VCR OSD Mono", 30, FlxColor.BLACK);
        //loginButton.label.y += 100;
        add(loginButton);

        errorText = new FlxText(registerButton.x, registerButton.y - 40, 0, "error bruh", 17);
